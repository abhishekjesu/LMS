...
def add_books(request):
    ...
    if request.method == "POST":
        ...
        book = BooksDB(title=title, author=author, publisher=publisher, language=language, copies=copies)
        if book is not None:
            book.save()
...
def delete_books(request):
    ...
    if request.method == "POST":
        ...
        delete_book = BooksDB.objects.get(id=B_id)
...
def search_book(request):
    ...
    if request.method == "POST":
        ...
        book = BooksDB.objects.get(id=bid)
        c = {"book":book}
